    ...
    return (
        <article className="save-manager">
            ...
            <label htmlFor="save-search">搜尋存檔名稱
                <input id="save-search" type="search" value={search} disabled={locked}
                       onChange={(event) => setSearch(event.target.value)} placeholder="輸入名稱"/>
            </label>
            ...
            <ul className="save-list" aria-label="全部存檔">
                ...
                <small>{save.ending === "departed" ? "已抵達北方" : save.ending === "postponed" ? "已約好延期" : "故事進行中"}{save.busy ? " · 回合處理中，暫不可刪除" : ""} · {save.id.slice(0, 8)}</small>
                        ...
                        <button disabled={locked} aria-label={`匯出 ${save.name}（${save.id.slice(0, 8)}）`}
                                onClick={() => exportOne(save)}>匯出
                        </button>
                    </li>
                ))}
            ...
            <div className="actions">
                <button className="danger" disabled={locked || !selected.length}
                        onClick={() => setConfirmation(saves.filter((save) => selected.includes(save.id)))}>
            ...
            <dialog ref={dialog} className="delete-dialog" aria-labelledby="delete-save-title"
                    onCancel={(event) => {
                        ...
                        if (!locked) setConfirmation([]);
                    }}>
                <h2 id="delete-save-title">刪除這 {confirmation.length} 個存檔？</h2>
                <p>故事進度、記憶、回合紀錄與連線權杖都會永久刪除。匯出的備份可重新匯入故事進度。</p>
                <ul>{confirmation.map((save) => <li key={save.id}>{save.name} · 第 {save.day} 天
                    · {save.id.slice(0, 8)}{save.id === currentId ? "（目前使用）" : ""}</li>)}</ul>
                {confirmation.some((save) => save.id === currentId) &&
                    <p>{confirmation.length === saves.length ? "刪除後將回到故事開始畫面。" : "刪除後將切換至另一個存檔。"}</p>}
                <div className="actions">
                    <button autoFocus disabled={locked} onClick={() => setConfirmation([])}>取消</button>
                    <button className="danger" disabled={locked}
                            onClick={remove}>{deleting ? "刪除中…" : "確認永久刪除"}</button>
                </div>
            </dialog>
        </article>
    );
