...
class NarrationContextTest {
    ...
    @Test
    fun `模型收到原始問題有效偏好與來源但不含舊副本或秘密`() {
        val fake = RecordingModel(listOf("""{"text":"你喜歡黑咖啡，我記著呢。"}"""))
        val memory = listOf(
            Memory("tea", MemoryLayer.SEMANTIC, "我喜歡無糖茶。", "tea"),
            Memory(
                "coffee",
        ...
        SpringStoryModel(ChatClient.create(fake), "live").narrate(
            ValidatedOutcome(World("test", "test", 26), true, "回想飲料偏好"),
    ...
    fun `威脅語氣會使用同一預算修復`() {
        val fake =
            RecordingModel(listOf("""{"text":"把你丟進河裡。"}""", """{"text":"無糖茶，記住了。我把糖罐移遠一點。"}"""))
        val budget = TurnBudget()
        val draft = SpringStoryModel(ChatClient.create(fake), "live").narrate(
            ValidatedOutcome(World("test", "test", 26), true, "偏好已保存"),
    ...
    fun `已出發事件保留作者事實且恢復時仍傳遞原始問題`() {
        ...
        val outcome =
            ValidatedOutcome(world, true, "渡船離岸，你們抵達北方星燈台。", "departure", speakers = listOf("Elia"))
        ...
        val result = KoogTurnWorkflow(WorldRules(emptyList()), model, EmbabelNpcPlanner()).execute(
            TurnWork("recovered", world, TurnCommand("req", 0, text = "我們出發吧。"), outcome)
        ) { _, _ -> error("不可重新提交") }
    ...
    @Test
    fun `自由聊天提示允許聊天但模糊指令仍要澄清`() {
        val fake = RecordingModel(listOf("""{"actionId":"chat"}"""))
        val intent = SpringStoryModel(ChatClient.create(fake), "live").understand(
            TurnCommand("req", 0, text = "先聊聊你今天的心情吧。"),
    ...
    @Test
    fun `回想不能改談北方或捏造已準備好的飲品`() {
        val fake = RecordingModel(
            listOf(
                """{"text":"我記得你喜歡無糖茶，我剛好有準備。"}""",
                """{"text":"無糖茶，我記得。口味改變時也可以再告訴我。"}"""
            )
        )
        val budget = TurnBudget()
        val draft = SpringStoryModel(ChatClient.create(fake), "live").narrate(
            ValidatedOutcome(World("test", "test", 26), true, "你喜歡無糖茶。"),
            "Elia",
            listOf(Memory("tea", MemoryLayer.SEMANTIC, "你喜歡無糖茶。", "tea")),
            budget,
            NarrationContext("還記得我喝什麼嗎？")
        )
