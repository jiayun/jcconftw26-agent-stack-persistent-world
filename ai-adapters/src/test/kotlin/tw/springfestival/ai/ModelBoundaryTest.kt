...
import tw.springfestival.domain.MemoryLayer
import tw.springfestival.domain.ValidatedOutcome
import tw.springfestival.domain.World

class ModelBoundaryTest {
    @Test
    fun `唯讀工具先過濾角色且共用三次上限`() {
        val b = TurnBudget()
        val tools = ReadOnlyKnowledge(
            "公開場景",
            listOf(
                Memory("private", MemoryLayer.SEMANTIC, "秘密", "private", setOf("Elia")),
                Memory("known", MemoryLayer.EPISODIC, "一起修燈", "festival")
            ),
            "Miro",
            b
        )
    ...
    fun `Spring AI 工具往返與模型請求都被計數`() {
        ...
        val fake = object : ChatModel {
            ...
            override fun call(prompt: Prompt): ChatResponse {
                calls++
                val message = if (calls == 1) AssistantMessage.builder().content("").toolCalls(
                    listOf(
        ...
        val draft = model.narrate(
            ValidatedOutcome(World("test", "測試", 0), true, "回想約定"),
            "Elia",
            listOf(Memory("promise", MemoryLayer.RELATIONSHIP, "北方約定", "promise")),
            budget
        )
    ...
    fun `持續要求工具的模型不能超過整回合預算`() {
        ...
        val fake = object : ChatModel {
            ...
            override fun call(prompt: Prompt): ChatResponse {
                ...
                return ChatResponse(
                    listOf(
                        Generation(
                            AssistantMessage.builder().content("").toolCalls(
                                listOf(
                                    AssistantMessage.ToolCall(
                                        "tool-$calls",
                                        "function",
        ...
        assertThrows(Exception::class.java) {
            SpringStoryModel(
                ChatClient.create(fake),
                "live"
            ).narrate(ValidatedOutcome(World("test", "測試", 0), true, "場景"), "Elia", emptyList(), budget)
        }
