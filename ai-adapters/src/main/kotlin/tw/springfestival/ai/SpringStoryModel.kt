...
import tw.springfestival.application.NarrationContext
import tw.springfestival.application.StoryModel
...
class OfflineChatModel : ChatModel {
    override fun call(prompt: Prompt): ChatResponse =
        ChatResponse(listOf(Generation(AssistantMessage("{\"text\":\"河風輕輕吹過，今天也有值得記住的小事。\"}"))))
}

class SpringStoryModel(
    private val liveClient: ChatClient?,
    override val mode: String,
    private val voices: Map<String, String> = mapOf(
        "Elia" to "細心、務實，以溫和幽默表達關心，尊重玩家。",
        "Miro" to "熱心、好奇，清楚區分已知紀錄與比喻。"
    )
) : StoryModel {
    ...
    override fun understand(command: TurnCommand, scene: Scene, budget: TurnBudget): PlayerIntent {
        ...
        ) return PlayerIntent(clarification = "船票與出發條件必須由世界紀錄確認。請到渡口查詢並完成訂位，再選擇合法的出發行動。")
        ...
        if (text.contains("記得") || text.contains("約定") || text in listOf(
                "聊天",
                "你好"
            )
        ) return PlayerIntent("chat")
        ...
        val allowedActions =
            scene.suggestions.map { it.id }.toSet() + setOf("chat", "rest") + Place.entries.map { "move:${it.name}" }
        val intentRules = """
        ...
        """.trimIndent()
        return callTyped(
            liveClient!!, intentRules, text, converter, budget,
            ReadOnlyKnowledge(scene.toString(), emptyList(), "player", budget),
            repairInstruction = "上一輪意圖格式或行動不合法。請重新判讀玩家原文：詢問角色意見應填 chat；只有明確要求執行才選允許的行動 ID；無法確定則 actionId=null 並填 clarification。"
        ) { intent ->
    ...
    override fun narrate(
        ...
        val user = """
            ...
            ${
            evidence.joinToString("\n") { "[${it.layer}; 來源=${it.sourceId}; 更新=${it.updatedRevision}; 更正=${it.correction != null}] ${it.effectiveText()}" }
                .ifEmpty { "無。不要引用其他往事或猜測偏好。" }
        }
        """.trimIndent()
        return callTyped(
            liveClient!!, system, user, BeanOutputConverter(NarrativeDraft::class.java), budget,
            ReadOnlyKnowledge("${current.place.label}：${outcome.text}", evidence, character, budget)
        ) { draft ->
            require(draft.text.isNotBlank() && draft.text.length <= 1500) { "請以 2 到 4 句完整回答。" }
            require(
                !Regex("沒見過世面|放鴿子|丟.{0,8}(河|海)|留.{0,8}雪地|不會等你|不會放過你|誰稀罕|誰在乎").containsMatchIn(
                    draft.text
                )
            ) {
            ...
            if (!Regex("準備|泡|煮|筆記本").containsMatchIn(outcome.text)) {
                require(
                    !Regex("(?:已經|剛好|偷偷|特地|順手|早就|我有).{0,16}(?:準備|泡|煮)|(?:準備|泡|煮).{0,12}(?:水|茶|咖啡|湯)|寫.{0,8}筆記本").containsMatchIn(
                        draft.text
                    )
                ) {
            ...
            if (recalled) {
                ...
                val value = preference?.effectiveText()
                    ?.let { Regex("(?:喜歡|選了)([^，；。！？]+)").find(it)?.groupValues?.get(1)?.trim() }

                fun normalized(s: String) =
                    s.replace("不加糖的茶", "無糖茶").replace("不加糖茶", "無糖茶").replace("不加糖的咖啡", "黑咖啡")
                if (value != null && value.length in 2..16) require(normalized(draft.text).contains(normalized(value))) { "先回答證據中的偏好，不要改談其他話題。" }
                if (evidence.isEmpty()) require(
                    Regex("不知道|沒有|不確定|不清楚|沒.{0,8}(記|說)|再.{0,8}(說|告訴)|不記得").containsMatchIn(
                        draft.text
                    )
                ) { "沒有相關記憶，請坦白說明並澄清。" }
            }
    ...
    private fun <T : Any> callTyped(
        client: ChatClient,
        system: String,
        user: String,
        converter: BeanOutputConverter<T>,
        budget: TurnBudget,
        knowledge: ReadOnlyKnowledge,
        repairInstruction: String = "上一輪未通過格式或演出規則檢查。請重新確認 JSON 格式、溫和語氣，直接回答證據中的偏好或表示沒有記憶；回應現在位置與選擇的行動，不得聲稱準備未記錄的飲品、物品或筆記本。",
        validate: (T) -> Unit = {}
    ): T {
        ...
        repeat(2) {
            val future = executor.submit(Callable {
                val response = client.prompt().advisors(BudgetAdvisor(budget)).tools(knowledge)
                    .system(system + "\n" + converter.format + repair).user(user).call().chatResponse()
                    ?: error("模型未回傳內容")
            ...
            try {
                return future.get(budget.remainingMs().coerceAtLeast(1), TimeUnit.MILLISECONDS)
            } catch (e: Exception) {
