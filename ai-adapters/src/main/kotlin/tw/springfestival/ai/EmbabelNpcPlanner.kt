...
import tw.springfestival.domain.NpcPlan
import tw.springfestival.domain.World
...
class EmbabelNpcPlanner : NpcPlanner {
    ...
    fun planGoal(w: World, npc: String, goal: String): NpcPlan {
        ...
        val facts = mapOf(
            "bridgeOpen" to ("bridgeClosed" !in w.flags), "ferryAvailable" to (w.weather() != "細雨"),
            "archiveOpen" to (w.tick % 3 != 2), "witnessAvailable" to (w.tick % 3 == 2),
            "truthKnown" to ("truth" in w.flags), "helpersKnown" to ("allies" in w.flags)
        )

        fun action(id: String, pre: List<String>, post: String, cost: Double) =
            ConditionAction(id, pre = pre, post = listOf(post), cost = { cost })

        val actions = when (goal) {
            "travel" -> listOf(
                action("check_bridge", listOf("bridgeOpen"), "routeKnown", .1),
                action("ask_ferry", listOf("ferryAvailable"), "ferryKnown", .2),
                action("propose_ferry", listOf("ferryKnown"), "routeKnown", .2),
                action("suggest_delay", emptyList(), "routeKnown", .9)
            )

            "investigate" -> listOf(
                action("read_archive", listOf("archiveOpen"), "recordFound", .1),
            ...
            "festival" -> listOf(
                action("prepare_tools", listOf("truthKnown"), "repairReady", .1),
        ...
        val target =
            mapOf("travel" to "routeKnown", "investigate" to "informationKnown", "festival" to "festivalPlan").getValue(
                goal
            )
        val conditions =
            (actions.flatMap { it.knownConditions }.toSet() + facts.keys).associateWith { facts[it] ?: false }
        val planner = DefaultPlannerFactory.createPlanner(
            ProcessOptions(),
            WorldStateDeterminer.fromMap(conditions.mapValues { ConditionDetermination(it.value) })
        )
