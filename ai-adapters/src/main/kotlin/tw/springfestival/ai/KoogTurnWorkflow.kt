...
private class NoDirectModelExecutor : PromptExecutor() {
    override suspend fun execute(prompt: Prompt, model: LLModel, tools: List<ToolDescriptor>): Message.Assistant =
        error("模型呼叫必須經 Spring AI")

    override fun executeStreaming(prompt: Prompt, model: LLModel, tools: List<ToolDescriptor>): Flow<StreamFrame> =
        error("不使用 token streaming")

    override suspend fun moderate(prompt: Prompt, model: LLModel): ModerationResult = error("模型呼叫必須經 Spring AI")
    override fun close() {}
}

class KoogTurnWorkflow(private val rules: WorldRules, private val model: StoryModel, private val planner: NpcPlanner) :
    TurnWorkflow {
    override fun execute(work: TurnWork, commit: (ValidatedOutcome, List<NpcPlan>) -> ValidatedOutcome): TurnResult =
        runBlocking {
            ...
            val graph = strategy<String, String>("persistent-player-turn") {
                val context by node<String, String>("read-world-and-knowledge") { input ->
                    nodes += "read-world-and-knowledge"; memories =
                    work.world.visibleMemories("player", work.command.text.orEmpty()); input
                }
                val understand by node<String, String>("understand-or-clarify") { input ->
                    nodes += "understand-or-clarify"
                    if (outcome == null) intent = try {
                        model.understand(work.command, rules.scene(work.world, model.mode), budget)
                    } catch (_: Exception) {
                        PlayerIntent(clarification = "剛才沒能確認你的意思。你想先聽聽角色的看法，還是採取行動？可以換個說法告訴我。")
                    }
                    input
                }
                ...
                val judge by node<String, String>("domain-validation") { input ->
                    nodes += "domain-validation"; outcome =
                    rules.resolve(work.world, intent, plans, work.turnId, work.command.text); input
                }
                val clarify by node<String, String>("clarification") { input ->
                    nodes += "clarification"; outcome =
                    ValidatedOutcome(work.world, false, intent.clarification ?: "請選擇一個明確行動。"); input
                }
                ...
                val perform by node<String, String>("spring-ai-performance") { input ->
                    ...
                    val context = NarrationContext(
                        work.command.text, event?.title, event?.lockedFacts.orEmpty(),
                    ...
                    narrative = try {
                        ...
                        fallback = true; saved.text
                    }
                    input
                }
                val fallbackNode by node<String, String>("author-fallback") { input ->
                    nodes += "author-fallback"; narrative = outcome!!.text; input
                }
            ...
            val agent = GraphAIAgent(
                NoDirectModelExecutor(),
                AIAgentConfig.withSystemPrompt("自訂節點執行世界回合", maxAgentIterations = 20),
                graph
            )
            try {
                agent.run(work.turnId)
            } finally {
                agent.close()
            }
            val saved = outcome!!
            TurnResult(
                ...
                saved.accepted,
                narrative,
                rules.scene(saved.world, model.mode),
                saved.memoryIds,
                ExecutionTrace(
                    nodes,
                    plans,
                    usedMemorySources.toList(),
                    (System.nanoTime() - budget.started) / 1_000_000,
                    budget.requests,
                    budget.tools,
                    budget.tokens,
                    fallback,
