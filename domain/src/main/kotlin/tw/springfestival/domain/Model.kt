...
enum class Place(val label: String) {
    INN("旅館"), MARKET("市集"), TEAHOUSE("茶書屋"), BRIDGE("舊橋"), DOCK("渡口"), LIGHTHOUSE(
        "北方星燈台"
    )
}
...
data class Memory(
    val id: String, val layer: MemoryLayer, val text: String, val sourceId: String,
...
data class Relationship(val trust: Int = 0, val closeness: Int = 0, val tension: Int = 0, val mood: String = "平靜") {
    fun description() = when {
        tension > 3 -> "願意聽你說，但有些拘謹"; closeness >= 5 -> "把你當成可以一起商量未來的人"; trust >= 3 -> "開始放心把心事交給你"; else -> "正在慢慢認識你"
    }
}
...
data class World(
    ...
    val contentVersion: String = CONTENT_VERSION,
    val revision: Long = 0,
    val tick: Int = 0,
    val place: Place = Place.INN,
    val chapter: Int = 0,
    ...
    val skills: Map<String, Set<String>> = mapOf(
        "cooking" to emptySet(),
        "observation" to emptySet(),
        "folkMagic" to emptySet()
    ),
    val memories: List<Memory> = emptyList(),
    val journal: List<JournalEntry> = emptyList(),
    val npcActivities: Map<String, String> = emptyMap(),
    ...
    fun skillLevel(skill: String) = when (skills[skill].orEmpty().size) {
        0, 1 -> "初識"; 2, 3 -> "熟悉"; else -> "拿手"
    }
...
data class Branch(
    ...
    val label: String,
    val text: String,
    val require: Set<String> = emptySet(),
    val flags: Set<String> = emptySet(),
    ...
    val layer: MemoryLayer = MemoryLayer.EPISODIC,
    val pinned: Boolean = false,
    val ending: String? = null
)

data class StoryEvent(
    val id: String, val category: String, val title: String, val place: Place,
...
data class Scene(
    val saveId: String, val name: String, val revision: Long, val day: Int, val period: String,
...
data class ExecutionTrace(
    ...
    val plans: List<NpcPlan>,
    val memorySources: List<String>,
    val elapsedMs: Long,
    val modelRequests: Int = 0,
    val toolCalls: Int = 0,
    val tokens: Long = 0,
    val fallback: Boolean = false,
    val stateDiff: Map<String, String> = emptyMap(),
