...
class WorldRules(val events: List<StoryEvent>) {
    init {
        ...
        events.forEach { e ->
            require(e.category in setOf("main", "character", "life", "memory", "atmosphere")) { "事件類別不正確：${e.id}" }
    ...
    }.sortedWith(compareByDescending<StoryEvent> { it.priority + if (it.id !in w.completed) 5 else (w.tick - w.completed.getValue(it.id)).coerceAtMost(30) }
    ...
    fun suggestions(w: World): List<Suggestion> {
        ...
        val choices = selected?.branches?.filter { w.flags.containsAll(it.require) }?.map { Suggestion("event:${selected.id}:${it.id}", it.label) }.orEmpty().toMutableList()
        ...
        if (selected?.id == "festival" && "allies" !in w.flags) {
            ...
            helper?.branches?.firstOrNull { "allies" in it.flags }?.let { branch -> choices += Suggestion("event:${helper.id}:${branch.id}", branch.label) }
        }
    ...
    fun scene(w: World, mode: String): Scene {
        val e = event(w)
        return Scene(w.id, w.name, w.revision, w.day(), w.period(), w.weather(), w.place, w.place.label,
            (w.npcPlaces().filterValues { it == w.place }.keys + e?.participants.orEmpty()).distinct(), w.chapter, e?.title,
            (e?.text?.let { text -> text + if (e.id == "festival" && w.skills.values.any { it.size >= 2 }) " 先前累積的生活練習，讓你辨認細節時更有把握。" else "" }) ?: "${w.place.label}的風帶著河水的氣味。你可以停留，也可以去別處看看。", suggestions(w),
            buildList { if ("promise" in w.flags) add("與 Elia 約定春祭後一起去北方"); if ("bridgeClosed" in w.flags) add("北橋已封閉"); if ("truth" in w.flags) add(WORLD_TRUTH); if ("ticket" in w.flags) add("已取得渡船船票"); if ("festivalDone" in w.flags) add("春祭已完成") },
            w.ending, w.relationships.mapValues { it.value.description() }, w.skills.mapValues { w.skillLevel(it.key) }, mode)
    }
    fun resolve(w: World, intent: PlayerIntent, plans: List<NpcPlan>, turnId: String, originalText: String? = null): ValidatedOutcome {
        ...
        if (action == "rest" || action.startsWith("move:")) {
            ...
            return ValidatedOutcome(next, true, if (action == "rest") "你讓肩膀放鬆，休息了一個時段。沒有人催促你。" else "你沿著河畔走到${place.label}。", "routine")
        }
        ...
        val memories = (w.memories + listOfNotNull(memory)).let { all -> all.filter { it.pinned } + all.filterNot { it.pinned }.takeLast(100) }
        ...
        val next = w.copy(revision = w.revision + 1, flags = w.flags + b.flags,
            completed = if (b.id == "listen") w.completed else w.completed + (e.id to w.tick), chapter = w.chapter + if (e.advanceChapter) 1 else 0,
            relationships = relations, skills = skills, memories = memories, ending = b.ending ?: w.ending,
    ...
    private fun advance(w: World, plans: List<NpcPlan>): World {
        ...
        val legal = setOf("check_bridge", "ask_ferry", "suggest_delay", "read_archive", "ask_witness", "inspect_lamp", "ask_helpers", "prepare_tools")
        return w.copy(tick = w.tick + 1, npcActivities = plans.groupBy { it.npc }.mapNotNull { (npc, p) -> p.firstNotNullOfOrNull { it.nextStep?.takeIf { a -> a in legal && when (a) {
