...
object MemoryRecall {
    private val vocabulary = linkedMapOf(
        "drink" to listOf("飲料", "飲品", "喝", "茶", "咖啡", "果汁", "可可", "牛奶", "豆漿", "無糖", "不加糖", "甜度", "熱湯", "drin", "tea", "coffee"),
        "food" to listOf("食物", "吃", "料理", "早餐", "晚餐", "口味", "麵包", "food", "eat"),
        "travel" to listOf("旅行", "北方", "出發", "去哪", "哪裡", "哪裏", "約定", "說好", "答應", "承諾", "延期", "船", "travel", "promise"),
        "festival" to listOf("星燈", "修燈", "配置", "負載", "祭典", "春祭", "燈架", "festival"),
        "joke" to listOf("玩笑", "笑話", "河風調味", "joke"),
        "letter" to listOf("信", "老闆", "值班", "letter"),
    )
    fun topics(text: String): Set<String> = vocabulary.filterValues { words -> words.any { text.contains(it, true) } }.keys
    ...
    fun isRecallQuestion(text: String) = listOf("記得", "記不記", "之前", "上次", "說過", "說好", "答應", "約定", "偏好", "喜歡什麼", "remember").any { text.contains(it, true) }
    private fun terms(text: String): Set<String> = Regex("[\\p{IsHan}]{2,}|[a-zA-Z]{3,}").findAll(text.lowercase())
        ...
        .filterNot { it in setOf("記得", "還記", "得我", "我喜", "喜歡", "歡喝", "什麼", "嗎呢", "我們", "之前", "你們", "現在", "說過", "the", "you") }.toSet()
    ...
    fun select(memories: List<Memory>, viewer: String, query: String = "", limit: Int = 6): List<Memory> {
        ...
        val eligible = memories.withIndex().filter { viewer in it.value.knownBy && it.value.layer != MemoryLayer.WORKING }
        ...
        return current.filter { query.isBlank() || relevance(it.value) > 0 }
            .sortedWith(compareByDescending<IndexedValue<Memory>> { relevance(it.value) }
                .thenByDescending { it.value.updatedRevision }.thenByDescending { it.value.importance }.thenByDescending { it.index })
            .take(limit).map { item -> item.value.let { if (it.correction != null) it.copy(text = it.effectiveText()) else it } }
    }
}
...
fun World.changeMemory(memoryId: String, replacement: String?): World {
    ...
    val target = memories.firstOrNull { it.id == memoryId && "player" in it.knownBy } ?: throw Missing("找不到這筆可見記憶。")
    val topic = MemoryRecall.preferenceTopic(target)
    val remaining = memories.filterNot { it.id == target.id ||
