...
class MemoryRecallTest {
    private val promise =
        Memory("promise", MemoryLayer.RELATIONSHIP, "我們約好春祭後去北方。", "promise", importance = 5, pinned = true)
    ...
    fun `新偏好與更正優先不回收舊文字`() {
        ...
        val changed =
            world.copy(revision = 2, memories = world.memories + newer).changeMemory("coffee", "我現在喜歡熱可可。")
        assertEquals(
            listOf("我現在喜歡熱可可。"),
            changed.visibleMemories("Elia", "喜歡喝什麼").map { it.effectiveText() })
    ...
    @Test
    fun `相同主題的新私人偏好不覆蓋另一角色已知版本`() {
        val privateCoffee =
            tea.copy(id = "private", text = "我喜歡咖啡。", knownBy = setOf("player", "Elia"), updatedRevision = 2)
    ...
    fun `多回合閒聊與別的記憶不影響回想`() {
        ...
        var w = rules.resolve(
            world,
            PlayerIntent("chat", memoryCandidate = "我喜歡無糖茶。"),
            emptyList(),
            "preference",
            "我喜歡無糖茶。"
        ).world
    ...
    @Test
    fun `共同回想不把玩家的私人記憶洩漏給在場角色`() {
        val privateTea = tea.copy(knownBy = setOf("player"))
        val reply = WorldRules(emptyList()).resolve(
            world.copy(memories = listOf(privateTea)),
            PlayerIntent("chat"),
            emptyList(),
            "q",
            "還記得我喝什麼嗎？"
