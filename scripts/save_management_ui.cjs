...
(async () => {
  ...
  try {
    ...
    const download = await downloadPromise; const backup = JSON.parse(await fs.readFile(await download.path(), 'utf8'));
    ...
    await page.getByRole('button', { name: '刪除已選的 1 個存檔', exact: true }).click();
    const turn = await api(`/saves/${c.saveId}/turns`, { requestId: 'stale-delete', expectedRevision: 0, suggestionId: 'rest' });
    for (let i = 0; i < 100; i++) {
