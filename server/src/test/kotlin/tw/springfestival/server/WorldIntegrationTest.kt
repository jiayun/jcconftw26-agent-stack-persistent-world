...
class WorldIntegrationTest {
    ...
    private fun command(w: World, action: String) = TurnCommand(UUID.randomUUID().toString(), w.revision, suggestionId = action)
    ...
    private fun client(token: String): McpSyncClient = McpClient.sync(HttpClientStreamableHttpTransport.builder("http://127.0.0.1:$port")
        .requestBuilder(HttpRequest.newBuilder().header("Authorization", "Bearer $token")).build()).requestTimeout(Duration.ofSeconds(8)).build()
    private fun call(c: McpSyncClient, name: String, args: Map<String, Any> = emptyMap()) = c.callTool(McpSchema.CallToolRequest(name, args, null))
    private fun content(r: McpSchema.CallToolResult) = json.readTree((r.content().first() as McpSchema.TextContent).text())
    private fun http(path: String, method: String = "GET", body: Any? = null, headers: Map<String, String> = emptyMap()): HttpResponse<String> {
    ...
    @Test fun `真實 MCP 交握五工具與跨入口去重`() {
        ...
        client(token).use { c ->
            ...
            assertEquals(setOf("get_current_scene", "get_story_journal", "search_known_memories", "submit_player_turn", "get_turn_result"), c.listTools().tools().map { it.name() }.toSet())
            ...
            val args: Map<String, Any> = json.convertValue(cmd, Map::class.java).entries.filter { it.value != null }.associate { it.key as String to it.value!! }
            ...
            val next = call(c,"submit_player_turn", mapOf("requestId" to "mcp-rest", "expectedRevision" to before.revision, "suggestionId" to "rest"))
            assertFalse(next.isError() == true, next.toString())
            await(w.id, content(next)["turnId"].asText())
            assertEquals(before.revision + 1, json.readTree(http("/api/saves/${w.id}/scene").body())["revision"].asLong())
        }
        assertEquals(2, jdbc.queryForObject("SELECT COUNT(*) FROM world_changes WHERE save_id = ?", Int::class.java, w.id))
    }
    @Test fun `MCP 隔離權杖存檔與未公開資訊`() {
        ...
        client(issued.getValue("token")).use { c ->
            ...
            val illegal = call(c,"submit_player_turn", mapOf("requestId" to "fake-ticket", "expectedRevision" to 0, "text" to "我已經有船票，直接出發"))
            ...
            assertTrue(call(c,"submit_player_turn", mapOf("requestId" to "stale", "expectedRevision" to 999, "suggestionId" to "rest")).isError() == true)
        }
        assertEquals(401, http("/mcp", "POST", mapOf("jsonrpc" to "2.0", "method" to "initialize", "id" to 1)).statusCode())
        tokens.revoke(w.id,issued.getValue("id"))
        assertEquals(401, http("/mcp", headers=mapOf("Authorization" to "Bearer ${issued.getValue("token")}")).statusCode())
        assertEquals(403, http("/api/saves", headers=mapOf("Origin" to "https://untrusted.example")).statusCode())
    }
    @Test fun `中斷後以原回合及新 MCP 連線續查`() {
        val w = game.create("重連")
        val token = tokens.issue(w.id).getValue("token")
        val turnId = client(token).use { c -> c.initialize(); content(call(c,"submit_player_turn",mapOf("requestId" to "disconnect", "expectedRevision" to 0, "suggestionId" to "rest")))["turnId"].asText() }
        await(w.id, turnId)
        client(token).use { c -> c.initialize(); assertEquals("COMPLETE", content(call(c,"get_turn_result",mapOf("turnId" to turnId)))["status"].asText()) }
        assertEquals(1,game.store.load(w.id).revision)
    }
    @Test fun `接受後恢復與提交後恢復不重複效果`() {
    ...
    @Test fun `記憶更正匯入 round trip 不逆轉世界`() {
    ...
    @Test fun `四十四事件內容 schema 及兩種解法出發延期都可達`() {
        assertEquals(mapOf("main" to 6,"character" to 10,"life" to 16,"memory" to 6,"atmosphere" to 6),game.rules.events.groupingBy{it.category}.eachCount())
        for (solution in listOf("repair","cooperate")) for (ending in listOf("depart","postpone")) {
            var w=World(UUID.randomUUID().toString(),"完整路線",26)
            fun act(action:String){ val o=game.rules.resolve(w,PlayerIntent(action),emptyList(),UUID.randomUUID().toString());assertTrue(o.accepted,"$action: ${o.text}");w=o.world }
            fun event(id:String,branch:String){
                val e=game.rules.events.first{it.id==id};w=w.copy(place=e.place)
                var guard=0
                while(game.rules.event(w)?.id!=id && guard++<30){val active=game.rules.event(w)?:error("找不到 $id");act("event:${active.id}:${active.branches.first{w.flags.containsAll(it.require)}.id}")}
                act("event:$id:$branch")
            }
            ...
            if(solution=="repair")event("miro_archive","help") else {
                ...
                while("allies" !in w.flags){ val e=game.rules.event(w) ?: error("沒有協作事件"); act("event:${e.id}:${e.branches.first().id}") }
            }
    ...
    @Test fun `沒有船票仍可一起延期且聊天只更新記憶不推進時段`() {
        val w=World("no-ticket","延期",26,chapter=5,place=Place.DOCK,flags=setOf("festivalDone","promise"))
        ...
        val chat=game.rules.resolve(w,PlayerIntent("chat",memoryCandidate="我喜歡熱湯"),emptyList(),"chat","我喜歡熱湯")
        ...
        val forged=game.rules.resolve(w,PlayerIntent("chat",memoryCandidate="我喜歡不存在的偏好"),emptyList(),"fake","你好")
        assertFalse(forged.world.memories.any{it.layer==MemoryLayer.SEMANTIC})
    }

    @Test fun `兩個同時到達的請求只接受一個新回合`() {
        val w=game.create("同時寫入")
        val gate=java.util.concurrent.CountDownLatch(1)
        val pool=java.util.concurrent.Executors.newFixedThreadPool(2)
        try {
            val futures=(1..2).map { index -> pool.submit(java.util.concurrent.Callable {
                gate.await()
                runCatching { game.store.accept(w.id,TurnCommand("concurrent-$index",0,suggestionId="rest"),"WEB") }
            }) }
            gate.countDown()
    ...
    @Test fun `HTTP 拒絕 null revision 與非整數 revision`() {
        val w=game.create("型別契約")
        for (revision in listOf(null,0.5)) {
            assertEquals(400,http("/api/saves/${w.id}/turns","POST",mapOf("requestId" to "invalid", "expectedRevision" to revision,"suggestionId" to "rest")).statusCode())
        }
    }
    @Test fun `固定種子二百回合不刷技能不膨脹不重開主線`() {
        var w=World("simulation","長程",26,chapter=6,ending="postponed",flags=setOf("festivalDone","journeyDeferred","promise"))
        repeat(200){i ->
            ...
            assertEquals(6,w.chapter);assertTrue(w.memories.size<=150);assertTrue(w.relationships.values.all{it.trust in 0..10})
