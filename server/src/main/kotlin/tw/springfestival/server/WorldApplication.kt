...
import tw.springfestival.ai.EmbabelNpcPlanner
import tw.springfestival.ai.KoogTurnWorkflow
import tw.springfestival.ai.SpringStoryModel
import tw.springfestival.application.*
import tw.springfestival.domain.StoryEvent
import tw.springfestival.domain.WorldRules

@SpringBootApplication
class WorldApplication {
    @Bean
    fun persistenceJson(): ObjectMapper =
        jacksonObjectMapper().enable(com.fasterxml.jackson.databind.DeserializationFeature.FAIL_ON_NULL_FOR_PRIMITIVES)
            .disable(com.fasterxml.jackson.databind.DeserializationFeature.ACCEPT_FLOAT_AS_INT)

    @Bean
    fun rules(json: ObjectMapper): WorldRules {
    ...
    @Bean
    fun worldStore(jdbc: JdbcTemplate, manager: PlatformTransactionManager, json: ObjectMapper): WorldStore =
        JdbcWorldStore(jdbc, TransactionTemplate(manager), json)
    ...
    fun storyModel(
        ...
        val characters: Map<String, Map<String, String>> =
            javaClass.getResourceAsStream("/content/characters.yaml").use { Yaml().load(it) }
        require(characters.keys == setOf("Elia", "Miro") && characters.values.all {
            it.keys.containsAll(
                setOf(
                    "voice",
                    "goal",
    ...
    @Bean
    fun npcPlanner(): NpcPlanner = EmbabelNpcPlanner()
    @Bean
    fun workflow(rules: WorldRules, model: StoryModel, planner: NpcPlanner): TurnWorkflow =
        KoogTurnWorkflow(rules, model, planner)

    @Bean(destroyMethod = "close")
    fun game(store: WorldStore, rules: WorldRules, model: StoryModel, workflow: TurnWorkflow) =
        GameService(store, rules, model, workflow)

    @Bean
    fun recover(game: GameService) = ApplicationRunner { game.recover() }
}
