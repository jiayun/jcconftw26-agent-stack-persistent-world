...
class ConnectionTokens(private val jdbc: JdbcTemplate) {
    ...
    fun authorize(header: String?): String? {
        ...
        return jdbc.query(
            "SELECT save_id FROM connection_tokens WHERE token_hash = ? AND revoked = FALSE",
            { rs, _ -> rs.getString(1) },
            hash(header.substring(7))
        ).firstOrNull()
    }

    fun list(saveId: String) = jdbc.query(
        "SELECT id, revoked FROM connection_tokens WHERE save_id = ?",
        { rs, _ -> mapOf("id" to rs.getString(1), "revoked" to rs.getBoolean(2)) },
        saveId
    )
...
class LocalAccessFilter(private val tokens: ConnectionTokens) : OncePerRequestFilter() {
    override fun doFilterInternal(request: HttpServletRequest, response: HttpServletResponse, chain: FilterChain) {
        ...
        if (request.serverName !in localHosts) {
            response.sendError(403, "只接受本機主機名稱。"); return
        }
        val origin = request.getHeader("Origin")
        if (origin != null && runCatching {
                val uri = URI(origin); uri.host in localHosts && uri.port == request.serverPort && uri.scheme == "http"
            }.getOrDefault(false).not()) {
            response.sendError(403, "此來源未授權。"); return
        }
        if (request.contentLengthLong > 2_000_000) {
        ...
        if (request.requestURI.startsWith("/mcp")) {
            ...
            if (saveId == null) {
                response.sendError(401, "連線權杖無效或已撤銷。"); return
            }
            request.setAttribute("authorizedSave", saveId)
        } else if (request.requestURI.startsWith("/api") && request.method in setOf(
                "POST",
                "PATCH"
