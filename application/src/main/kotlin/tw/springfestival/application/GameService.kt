...
data class TurnRecord(
    val id: String, val saveId: String, val command: TurnCommand, val status: String,
...
class GameService(
    ...
    val model: StoryModel,
    private val workflow: TurnWorkflow
) : AutoCloseable {
    ...
    fun create(name: String, demo: Boolean = false, seed: Long = 26): World {
        ...
        if (demo) {
            w = w.copy(
                chapter = 3, tick = 4, place = Place.BRIDGE, flags = setOf("tea", "joke", "promise", "anomaly"),
                completed = mapOf("arrival" to 0, "promise" to 1, "anomaly" to 2, "elia_recipe" to 3),
                relationships = mapOf(
                    "Elia" to Relationship(trust = 4, closeness = 3),
                    "Miro" to Relationship(trust = 1)
                ),
                skills = mapOf(
                    "cooking" to setOf("elia_recipe"),
                    "observation" to setOf("anomaly"),
                ...
                journal = listOf(
                    JournalEntry(
                        "promise",
                        "範例存檔的共同經歷",
                        "已認識 Elia、約好北方旅行，也發現星燈異常；尚未取得船票。",
                        0
                    )
                )
            )
    ...
    fun recover() {
        store.pending().forEach(::schedule)
    }
    ...
    private fun schedule(record: TurnRecord) {
        ...
        workers.submit {
            try {
                ...
                ) { outcome, plans -> store.commit(record, outcome, plans) }
                store.finish(record, result)
            } catch (_: Conflict) {
                store.fail(record, "REVISION_CONFLICT：場景已更新，請重新選擇行動。")
            } catch (_: Exception) {
                store.fail(record, "PROCESSING_FAILED：回合無法完成；已提交的世界會保留，可在重新啟動後補回演出。")
            } finally {
    ...
    fun import(world: World): World {
        ...
        require(
            world.relationships.keys == setOf(
                "Elia",
                "Miro"
            ) && world.relationships.values.all { it.trust in 0..10 && it.closeness in 0..10 && it.tension in 0..10 })
