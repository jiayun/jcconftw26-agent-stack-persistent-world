...
本專案已建立可啟動、可通關、可持續保存的遊戲與 MCP Server。驗收範圍包含本機啟動、遊戲路線、模型與規劃邊界、存檔恢復、MCP 跨入口操作、內容擴充及人工遊玩品質。**整體驗收尚未全部完成。** 以下區分已實作、已測試與仍需外部條件的驗收，重現步驟見 [測試指南](testing.md)。

三框架共同執行、世界不變量與回合恢復測試，提供本機單人應用的實作證據。公開部署所需的身分治理、跨服務恢復、維運與 production 驗證不在本版驗收範圍；真實模型的人工品質驗收另列於下方。
